	...
	class ezSQL_mssql extends ezSQLcore
	{
		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost')
		{
			...
			if ( ! $dbuser )
			{
				$this->register_error($ezsql_mssql_str[1].' in '.basename(__FILE__).' on line '.__LINE__);
			...
			{
				$this->register_error($ezsql_mssql_str[2].' in '.basename(__FILE__).' on line '.__LINE__);
		...
		*/
		...
		function select($dbname='')
		{
			...
			if ( ! $dbname )
			{
				$this->register_error($ezsql_mssql_str[3].' in '.basename(__FILE__).' on line '.__LINE__);
			...
			{
				$this->register_error($ezsql_mssql_str[4].' in '.basename(__FILE__).' on line '.__LINE__);
			...
			else if ( !@mssql_select_db($dbname,$this->dbh) )
			{
				...
				$this->register_error($str.' in '.basename(__FILE__).' on line '.__LINE__);
...
/**
 * Converts MsSQL DATETIME field to UNIX time value.
 *
 * @param string $mysqlstring Time from mysql DATETIME field.
 * @return time() compatible value.
 */
function sqldate2time( $mssqlstring ) {
	$m = str_replace( "  ", " ", $mssqlstring );
	if ( empty( $m ) )
		return false;
	...
	$day = (int) $dateArr[1];
	$year = (int) $dateArr[2];
	$hour = (int) substr( $dateArr[3], 0, 2 );
	$hour += (substr( $dateArr[3], strlen( $dateArr[3] )-2, 2 )=="PM") ? 12 : 0;
	$hour -= $dateArr[3]=="12:00AM" ? 12 : 0;
	$minute = (int) substr( $dateArr[3], strpos( $dateArr[3], ":" )+1, 2 );
