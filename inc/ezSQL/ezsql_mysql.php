	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
		...
		/**********************************************************************
		*  Destructor - used to close connection to databse
		*/
		function __destruct()
		{
			if ( $this->dbh )
		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost')
		{
			...
			if ( ! $dbuser )
			{
				$this->register_error($ezsql_mysql_str[1].' in '.basename(__FILE__).' on line '.__LINE__);
			...
			{
				$this->register_error($ezsql_mysql_str[2].' in '.basename(__FILE__).' on line '.__LINE__);
		...
		*/
		...
		function select($dbname='')
		{
			...
			if ( ! $dbname )
			{
				$this->register_error($ezsql_mysql_str[3].' in '.basename(__FILE__).' on line '.__LINE__);
			...
			{
				$this->register_error($ezsql_mysql_str[4].' in '.basename(__FILE__).' on line '.__LINE__);
...
/**
 * Converts MySQL DATETIME field to UNIX compatible time.
 *
 * @param string $mysqlstring Time from mysql DATETIME field.
 * @return time() compatible value.
 */
function sqldate2time( $mysqlstring ) {
	$m = $mysqlstring;
	if ( empty( $m ) )
		return false;
