...
// get all users
$List = $db->get_results(
	"SELECT TagID, TagName, (SELECT count(*) FROM BesedilaTags BT WHERE BT.TagID=T.TagID) AS TagCount
	FROM Tags T
	". ($_GET['Find'] == "" ? " " : " WHERE TagName LIKE '%". $db->escape($_GET['Find']) ."%'" ) ."
...
if ( count( $List ) == 0 ) {
	...
	while ( $i < $EndR ) {
		...
		if ( contains($ActionACL,"R") ) {
			...
			echo "<td><a href=\"javascript:void(0);\" onclick=\"loadTo('Edit','edit.php?Izbor=".$_GET['Izbor']."&Action=".$_GET['Action']."&ID=$Item->TagID');\"><b>". left($Item->TagName,30).(strlen($Item->TagName)>30?"...":"")."</b></a></td>\n";
			echo "<td align=\"right\" class=\"f10\">". $Item->TagCount ."</td>\n";
			echo "<td align=\"right\" valign=\"top\" width=\"20\">";
			if ( contains($ActionACL,"D") )
				echo "<a href=\"javascript:void(0);\" onclick=\"javascript:check('$Item->TagID','$Item->TagName');\"><img src=\"pic/list.delete.gif\" width=11 height=11 alt=\"Delete\" border=\"0\" align=\"absmiddle\" class=\"icon\"></a>";
			else
				echo "&nbsp;";
			echo "</td>\n";
			echo "</tr>\n";
