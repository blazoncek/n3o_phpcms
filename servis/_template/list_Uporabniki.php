...
if ( count( $List ) == 0 ) {
	...
	while ( $i < $EndR ) {
		...
		if ( $Item->UserID != 1 || contains($ActionACL,"D") ) {
			...
			echo "<td><a href=\"javascript:void(0);\" onclick=\"loadTo('Edit','edit.php?Izbor=".$_GET['Izbor']."&Action=".$_GET['Action']."&ID=$Item->UserID');\"><b>".left($Item->Name,30).(strlen($Item->Name)>30?"...":"")."</b></a>".(($Item->Active)? "":"*")."</td>\n";
			echo "<td align=\"center\" class=\"f10\">$Item->UserName</td>\n";
			echo "<td align=\"right\" valign=\"top\" width=\"20\">";
			if ( $Item->UserID > 1 && contains($ActionACL,"D") )
				echo "<a href=\"javascript:void(0);\" onclick=\"javascript:check('$Item->UserID','$Item->Name');\"><img src=\"pic/list.delete.gif\" width=11 height=11 alt=\"BriĹˇi\" border=\"0\" align=\"absmiddle\" class=\"icon\"></a>";
			else
