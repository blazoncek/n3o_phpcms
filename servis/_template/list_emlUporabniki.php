...
if ( count( $List ) == 0 ) {
	...
	while ( $i < $EndR ) {
		// get list item
		$Item = $List[$i++];
		if ( contains($ActionACL,"X") ) {
			// row background color
			$BgCol = $BgCol=="white" ? "#edf3fe" : "white";
			echo "<tr bgcolor=\"$BgCol\">\n";
			echo "<td><a href=\"javascript:void(0);\" onclick=\"loadTo('Edit','edit.php?Izbor=".$_GET['Izbor']."&Action=".$_GET['Action']."&ID=$Item->emlMemberID');\"><b>".left($Item->Naziv,30).(strlen($Item->Naziv)>30?"...":"")."</b></a>".(($Item->Aktiven)? "":"*")."</td>\n";
			echo "<td align=\"center\" class=\"f10\">$Item->Email</td>\n";
			echo "<td align=\"right\" valign=\"top\" width=\"20\">";
			if ( contains($ActionACL,"D") )
				echo "<a href=\"javascript:void(0);\" onclick=\"javascript:check('$Item->emlMemberID','$Item->Naziv');\"><img src=\"pic/list.delete.gif\" width=11 height=11 alt=\"Delete\" border=\"0\" align=\"absmiddle\" class=\"icon\"></a>";
			else
				echo "&nbsp;";
			echo "</td>\n";
			echo "</tr>\n";
