	...
	$("form[name='Vnos']").submit(function(){
		...
		$(this).ajaxSubmit({
			...
			beforeSubmit: function( formDataArr, jqObj, options ) {
				...
				if (empty(fObj.Naziv))	{alert("Please enter message title!"); fObj.Naziv.focus(); return false;}
...
<?php if ( contains($ACL,"W") ) : ?>
		<A HREF="javascript:void(0);" ONCLICK="loadTo('Edit','inc.php?Izbor=emlMessageTxt&Action=<?php echo $Action->ActionID ?>&emlMessageID=<?php echo $_GET['ID'] ?>')" TITLE="Add"><IMG SRC="pic/control.add_document.gif" ALIGN="absmiddle" WIDTH=14 HEIGHT=14 ALT="Add" BORDER="0" CLASS="icon"></A>&nbsp;:
<?php endif ?>
		Content</LEGEND>
<?php
		$List = $db->get_results(
		...
		if ( !$List ) 
			...
			foreach ( $List as $Item ) {
				...
				echo "<TD><A HREF=\"javascript:void(0);\" ONCLICK=\"loadTo('Edit','inc.php?Izbor=emlMessageTxt&Action=".$Action->ActionID."&emlMessageID=".$_GET['ID']."&ID=".$Item->ID."');\"><B>$Item->Naziv</B></A></TD>\n";
