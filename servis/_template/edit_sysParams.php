...
<?php
	...
	if ( !$Nazivi ) {
		...
		foreach ( $Nazivi as $Naziv ) {
			...
			echo "<TD><A HREF=\"javascript:void(0);\" ONCLICK=\"$('#editTekst').load('inc.php?Action=".$_GET['Action']."&Izbor=sysParametersTxt&Jezik=$Naziv->Jezik&ID=".$_GET['ID']."')\"><B>".left($Naziv->Naziv,45).((strlen($Naziv->Naziv)>45)? "...": "")."</B></A></TD>\n";
