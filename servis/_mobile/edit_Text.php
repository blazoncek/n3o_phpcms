...
<?php else : ?>
	...
	$("input[name!='newtag']:text, input[type=date], textarea, select").change(function(e){
		...
		if (fObj.name=="Ime" && fObj.value.length==0) {alert("Please enter name!"); fObj.focus(); return false;}
	...
	$("input[name='newtag']").change(function(e){
		...
		URL = '<?php echo dirname($_SERVER['PHP_SELF']) ?>/inc.php?Izbor=txtTags&BesediloID=<?php echo $_GET['ID'] ?>';
	...
	$("input[name='Find']").change(function(e){
		...
		var fObj = this;	// input object
		if (fObj.value.length==0) {return false;}
		URL = '<?php echo dirname($_SERVER['PHP_SELF']) ?>/inc.php?Izbor=txtRelated&BesediloID=<?php echo $_GET['ID'] ?>';
		$.mobile.changePage(URL, {
			reloadPage: true,
			type: "get",
			data: $(this).serialize()
...
if ( (int)$_GET['ID'] != 0 ) {
	...
	if ( $List ) foreach ( $List as $Naziv ) {
		echo "<li>";
		echo (contains($ACL,"W")? "<a href=\"inc.php?Izbor=txtText&BesediloID=".(int)$_GET['ID']."&ID=$Naziv->ID\">": "");
	...
	echo "<li data-icon=\"add\"><a href=\"inc.php?Izbor=txtText&BesediloID=". (int)$_GET['ID'] ."&ID=0\" title=\"Dodaj vsebino\" class=\"ui-btn-right\" data-iconpos=\"left\" data-icon=\"add\">Dodaj vsebino</a></li>\n";
	echo "</ul>\n";
	echo "</fieldset>\n";
	...
	if ( $List) foreach ( $List as $Item ) {
		echo "<li>";
		echo (contains($ACL,"W") ? "<a href=\"inc.php?Izbor=txtTags&BesediloID=". (int)$_GET['ID'] ."\" data-ajax=\"false\" data-theme=\"c\">" : "");
		echo $Item->TagName;
		echo (contains($ACL,"W") ? "</a>" : "");
		echo "</li>\n";
	}
	...
	if ( $List) foreach ( $List as $Item ) {
		echo "<li>";
		echo (contains($ACL,"W") ? "<a href=\"inc.php?Izbor=txtRelated&BesediloID=". (int)$_GET['ID'] ."\" data-ajax=\"false\" data-theme=\"c\">" : "");
		echo $Item->Ime;
		echo (contains($ACL,"W") ? "</a>" : "");
		echo "</li>\n";
	}
