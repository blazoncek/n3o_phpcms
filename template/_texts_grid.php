<?php
/* _besedila_grid.php - Display grid of texts.
.---------------------------------------------------------------------------.
...
// show all available texts in a category (grid view)
	...
	if ( $Besedila ) {
		...
		foreach ( $Besedila as $Besedilo ) {
			...
			echo ReplaceSmileys(PrependImagePath(substr($Tekst->Povzetek,0,127).(strlen($Tekst->Povzetek)>127?'...':''), "$WebPath/"), "$WebPath/pic/");
			echo "</div>\n";
